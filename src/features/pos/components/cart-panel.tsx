...
export function CartPanel() {
  ...
  if (cartItems.length === 0) {
    return <EmptyState title={t('pos.empty_cart_title')} description={t('pos.empty_cart_desc')} />
  }
          ...
          <div key={item.productId} className="rounded-2xl border p-4 group">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-1 opacity-100 xl:opacity-0 group-hover:opacity-100 transition-opacity">
                <Button aria-label={t('pos.edit_item', { name: item.name })} variant="ghost" size="icon-sm" onClick={() => setEditingItem(item)}>
                  <Edit2 className="size-4" aria-hidden="true" />
                </Button>
                <Button aria-label={t('pos.remove_from_cart', { name: item.name })} variant="ghost" size="icon-sm" onClick={() => removeItem(item.productId)}>
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Button aria-label={t('pos.decrease_qty', { name: item.name })} variant="outline" size="icon-sm" onClick={() => decreaseQty(item.productId)}>
                  <Minus className="size-4" aria-hidden="true" />
                </Button>
                <span className="min-w-8 text-center text-sm font-medium">{item.qty}</span>
                <Button aria-label={t('pos.increase_qty', { name: item.name })} variant="outline" size="icon-sm" onClick={() => increaseQty(item.productId)}>
