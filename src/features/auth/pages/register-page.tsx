...
export function RegisterPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const existingUser = await localDb.users.where('email').equals(email).first()
      if (existingUser) {
          ...
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <CardDescription>Mulai kelola bisnis Anda sekarang.</CardDescription>
              </div>
          ...
          <CardContent>
            <form id="register-form" className="flex flex-col gap-4" onSubmit={handleSubmit}>
              ...
              <label className="flex flex-col gap-2 text-sm font-medium" htmlFor="email">
                Email
                <Input id="email" name="email" type="email" placeholder="owner@usaha.co.id" autoComplete="email" spellCheck={false} disabled={loading} required />
              </label>
              <label className="flex flex-col gap-2 text-sm font-medium" htmlFor="password">
                Kata sandi
                <Input id="password" name="password" type="password" placeholder="Buat kata sandi" autoComplete="new-password" disabled={loading} required />
              </label>
